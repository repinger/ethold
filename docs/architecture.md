...
- **Background Tasks:** Two goroutines run in the background when Telegram is active:
  ...
  - The notification poller checks ETHOL system alerts. On startup, it captures existing unread notifications as a baseline to suppress historical alerts. Presence scanning operates independently via schedule windows; `PRESENSI` system notifications are acknowledged without triggering scans or alerts. Task (`TUGAS`) and other system notifications send a Telegram alert.
- **Locks:**
