...
The bot operates as a single-message interface to keep the chat clutter-free. Tapping inline keyboard buttons seamlessly updates the active message in-place via Telegram's `editMessageText` API without producing new messages or notification alerts. When commands are typed manually, the user's message and the previous bot message are removed in the background, keeping only the latest interaction visible.
