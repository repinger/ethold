...
When configured with `TELEGRAM_COMMAND_THREAD_ID`, the bot only accepts and replies to commands within the specified forum topic thread. Commands or button presses sent outside that topic are ignored or rejected. When configured with `TELEGRAM_NOTIF_THREAD_ID`, commands and button presses sent within the notification topic thread are rejected.
